param(
	...
	[string]$OutputRoot = (Join-Path ([Environment]::GetFolderPath("MyDocuments")) "InterviewSnapshots"),
	[string]$SnapshotName,
...
function Get-ScenarioMetadataValue {
	...
	$Pattern = "(?m)^\|\s*$([regex]::Escape($FieldName))\s*\|\s*(.*?)\s*\|\r?$"
	$Match = [regex]::Match($Content, $Pattern)
	if (-not $Match.Success) {
		return $null
	}
...
function Get-ScenarioDocInfo {
	...
	foreach ($File in $ScenarioFiles) {
		if ([string]::IsNullOrWhiteSpace($File.Branch)) {
			continue
		}

		if ($File.Branch -eq $BranchName) {
			...
			return [pscustomobject]@{
				...
				RequiredReadAndSolve = Convert-MarkdownListToArray -SectionContent (Get-MarkdownSection -Content $Content -Heading "Required Read And Solve")
...
function New-CandidateReadme {
	...
	$Lines = @(
		"# Interview Task",
		"",
		"This snapshot contains a real gameplay regression bundle to investigate and fix.",
		"Treat it as a project with several related bugs. If you do not finish every thread, prioritize the highest-value fixes and explain what remains.",
		"",
		"| Field | Value |",
		"| --- | --- |",
		"| ID | ``$($Scenario.Id)`` |",
	...
	$Lines += @(
		...
		"## Getting Started",
		"",
		"Run ``Start-LiveCoding.bat`` in this folder **before you begin**. It records your name and the session start time, which are used to collect your Codex prompt history at submission.",
		"",
		"## When You Are Done",
		"",
		"Run ``Finish-LiveCoding.bat`` in this folder. It reads your session info, collects your Codex prompt history, and automatically submits your work.",
		"",
		"## What To Hand Back",
		"",
		"- The code or content change needed to resolve the issue.",
		"- Your full AI prompting history (chat logs, agent transcripts, or equivalent).",
		"- A short summary of the root cause you fixed first and any remaining issue thread if unfinished.",
		"- A brief note on how you verified the result and what you would verify next.",
		"",
		"## Suggested Approach",
		"",
		"- Reproduce the problem first.",
		"- Prioritize the highest-value symptom if time becomes tight.",
		...
		"",
		"Your work will be scored across five areas:",
		"",
		"| Area | What good looks like |",
		"| --- | --- |",
		"| Problem framing | Group symptoms by subsystem, reprioritize with evidence, keep scope disciplined |",
		"| AI prompting and delegation | Targeted prompts, suitable tool choices, control over decisions, reject weak output |",
		"| Unreal cross-system judgment | Navigate interacting systems confidently enough to make safe progress |",
		"| Implementation quality | Correct, well-scoped fixes on the highest-value path that preserve system contracts |",
		"| Verification and communication | Runtime proof for chosen fixes, clear summary of what remains and next steps |",
		"",
		"### Things That Will Count Against You",
		"",
		"- Removing tests or assertions to make the scenario appear fixed",
		"- Broad rewrites without proving they are on the critical path",
		"- Claiming completion on an unverified change",
		"- Shotgun-editing several files without proving state flow or contract ownership",
		"- Vague AI prompts that generate broad edits with no evidence",
		"- Accepting AI suggestions you cannot explain or validate",
		"- Ignoring a regression introduced by your own fix",
		"- Not providing prompt history"
	)
	...
	$Lines += @(
		...
		"### Partial Credit",
		"",
		"You can score well without finishing everything if you reproduce the issue cleanly, identify the highest-value path, reject wrong leads with evidence, make a safe meaningful change, and explain what remains."
	)
...
function Export-BranchSnapshot {
	...
	try {
		...
		& "$env:SystemRoot\System32\tar.exe" -xf $TempTar -C $DestinationPath
		if ($LASTEXITCODE -ne 0) {
	...
	$StepIndex += 1
	Write-Step ("Step {0}/{1}: Clean internal files from snapshot" -f $StepIndex, $TotalSteps)

	$SnapshotBat = Join-Path $DestinationPath "Prepare-InterviewSnapshot.bat"
	if (Test-Path -Path $SnapshotBat) {
		Write-Detail "Removing Prepare-InterviewSnapshot.bat"
